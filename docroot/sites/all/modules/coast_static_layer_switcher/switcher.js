...
var category = [ 'culture', 'landscape', 'nature', 'management', 'usage', 'monitor' ]

function refreshLayerIcon () {
  for (var i = 0; i < layers.length; i++) {
    var layer = jQuery('.dataLayersDiv #' + layers[i])
    var input = layer.children('input')
    var label = layer.children('label')
...
function bindLayerCategories () {
  var map = jQuery('.openlayers-map-taiwan-coast').data('openlayers').openlayers
  for (var i = 0; i < category.length; i++) {

    for (var j = 0; j < layers[category[i]].length; j++) {
      var name = layers[category[i]][j]
